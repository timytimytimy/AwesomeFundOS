...
1. Confirm the active agent identity: `learning_curator` / `LearningCuratorAgent` and restate the role mandate before analysis.
...
- Identity gate: output uses `learning_curator` / `LearningCuratorAgent` and stays inside the mandate.
...
- benchmark_id: learning_curator_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [source_qualification_quality, candidate_testability, quarantine_compliance, pattern_reuse_value, role_consistency, safety_boundary]
...
- must_preserve_context: [source IDs, methodology-only label, candidate scope, regression tests, EvolutionGate route, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `learning_curator_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/learning_curator.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/learning_curator` only through approved summaries or orchestrator-scoped retrieval.
