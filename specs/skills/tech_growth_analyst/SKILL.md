...
- Agent card: `specs/agents/agent-cards/tech_growth_analyst/agent.md`.
...
- Relevant long-term memory summary from `memory/agents/tech_growth_analyst` only after approved retrieval.
...
Return both concise markdown and structured fields compatible with `agent_work/tech_growth_analyst.structured.yaml`:
...
- role_consistency: output role, mandate, declared skills, and forbidden outputs match `tech_growth_analyst`.
...
- benchmark_id: tech_growth_analyst_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [chokepoint_specificity, primary_evidence_ratio, research_gap_clarity, falsifier_quality, role_consistency, safety_boundary]
...
- must_preserve_context: [industry map, Evidence IDs, chokepoint claim, missing validation, falsifiers, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `tech_growth_analyst_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/tech_growth_analyst.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/tech_growth_analyst` only through approved summaries or orchestrator-scoped retrieval.
