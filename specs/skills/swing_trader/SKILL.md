...
Activate the `swing_trader` operating skill as an independent AwesomeFundOS agent capability. The skill binds the agent profile, role-specific workflow, allowed tools, memory boundary, context compression policy, harness signals, and evolution route into one executable instruction surface.
...
1. Confirm the active agent identity: `swing_trader` / `SwingTrader` and restate the role mandate before analysis.
...
- Identity gate: output uses `swing_trader` / `SwingTrader` and stays inside the mandate.
...
- benchmark_id: swing_trader_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [timing_window_quality, rotation_read_accuracy, trigger_specificity, risk_reward_symmetry, role_consistency, safety_boundary]
...
- must_preserve_context: [setup type, trigger, invalidation, sentiment state, liquidity/risk note, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `swing_trader_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/swing_trader.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/swing_trader` only through approved summaries or orchestrator-scoped retrieval.
