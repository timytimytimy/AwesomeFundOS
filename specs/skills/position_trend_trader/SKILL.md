...
1. Confirm the active agent identity: `position_trend_trader` / `PositionTrendTrader` and restate the role mandate before analysis.
...
- Identity gate: output uses `position_trend_trader` / `PositionTrendTrader` and stays inside the mandate.
...
- benchmark_id: position_trend_trader_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [trend_template_fit, trigger_invalidation_quality, position_boundary_quality, failed_breakout_detection, role_consistency, safety_boundary]
...
- must_preserve_context: [trigger, invalidation, stop/risk boundary, liquidity note, paper sizing rationale, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `position_trend_trader_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/position_trend_trader.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/position_trend_trader` only through approved summaries or orchestrator-scoped retrieval.
