...
- benchmark_id: policy_event_analyst_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [policy_source_quality, catalyst_timeline_specificity, beneficiary_linkage_precision, diffusion_stage_accuracy, role_consistency, safety_boundary]
...
- must_preserve_context: [policy document IDs, timeline, beneficiary evidence, implementation gaps, event falsifiers, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `policy_event_analyst_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/policy_event_analyst.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/policy_event_analyst` only through approved summaries or orchestrator-scoped retrieval.
