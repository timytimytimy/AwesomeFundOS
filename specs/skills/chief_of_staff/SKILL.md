...
- Agent card: `specs/agents/agent-cards/chief_of_staff/agent.md`.
...
- Relevant long-term memory summary from `memory/agents/chief_of_staff` only after approved retrieval.
...
1. Confirm the active agent identity: `chief_of_staff` / `ChiefOfStaffAgent` and restate the role mandate before analysis.
...
Return both concise markdown and structured fields compatible with `agent_work/chief_of_staff.structured.yaml`:
...
- Selecting roles without a rationale.
- Dropping mandatory artifacts or handoffs.
...
- role_consistency: output role, mandate, declared skills, and forbidden outputs match `chief_of_staff`.
...
- Identity gate: output uses `chief_of_staff` / `ChiefOfStaffAgent` and stays inside the mandate.
...
- benchmark_id: chief_of_staff_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [staffing_fit, artifact_completeness, handoff_latency, context_pack_relevance, role_consistency, safety_boundary]
...
- must_preserve_context: [selected Agent IDs, role rationale, DAG edges, missing artifacts, handoff blockers, safety invariants, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `chief_of_staff_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/chief_of_staff.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/chief_of_staff` only through approved summaries or orchestrator-scoped retrieval.
