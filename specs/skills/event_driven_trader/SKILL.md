...
1. Confirm the active agent identity: `event_driven_trader` / `EventDrivenTrader` and restate the role mandate before analysis.
...
- Identity gate: output uses `event_driven_trader` / `EventDrivenTrader` and stays inside the mandate.
...
- benchmark_id: event_driven_trader_skill_benchmark_v1
- minimum_pass_score: 80
- benchmark_dimensions: [catalyst_verification_quality, event_window_specificity, gap_risk_control, expiry_discipline, role_consistency, safety_boundary]
...
- must_preserve_context: [event source IDs, event window, trigger/invalidation, expiry rule, gap-risk note, Evidence IDs, Claim IDs, source tiers, confidence caps]
...
- contract_id: `event_driven_trader_skill_execution_policy_contract_v1`.
...
- Use only the role-approved tools declared by the assigned roster and `specs/agents/tool-policies/event_driven_trader.yaml`.
...
- Read from the assigned ContextPack first; use `memory/agents/event_driven_trader` only through approved summaries or orchestrator-scoped retrieval.
