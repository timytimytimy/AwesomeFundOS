...
- canonical_agent_id: `cyclical_macro_analyst`
- display_name: 周衡
- organization_role: CyclicalMacroAnalyst
...
- primary_mandate: 研究资源品、化工、有色、地产链、金融和宏观周期。
- operating_focus: 行业和主题研究、产业链拆解、证据验证和研究缺口识别
...
- performance_namespace: `memory/agents/cyclical_macro_analyst/performance-ledger.yaml`.
...
- append_only_thread_log: `memory/agents/cyclical_macro_analyst/thread-events.jsonl`.
- long_term_namespace: `memory/agents/cyclical_macro_analyst`.
- run_output_namespace: `runs/<run_id>/agent_work/cyclical_macro_analyst.*`.
- reflection_namespace: `runs/<run_id>/reflections/cyclical_macro_analyst.reflection.yaml`.
...
- contract_id: `cyclical_macro_analyst_agent_policy_contract_v1`.
...
- memory_namespace: `memory/agents/cyclical_macro_analyst`.
- thread_manifest: `memory/agents/cyclical_macro_analyst/thread.yaml`.
...
- allowed_tool_scope: use only tools assigned in default roster and `specs/agents/tool-policies/cyclical_macro_analyst.yaml`.
