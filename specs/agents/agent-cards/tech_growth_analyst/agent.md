...
- Persistent thread manifest: `memory/agents/tech_growth_analyst/thread.yaml`.
- Append-only thread event log: `memory/agents/tech_growth_analyst/thread-events.jsonl`.
...
- Long-term namespace: `memory/agents/tech_growth_analyst`.
- Run-specific outputs live under `runs/<run_id>/agent_work/tech_growth_analyst.*`.
- Reflections live under `runs/<run_id>/reflections/tech_growth_analyst.reflection.yaml`.
