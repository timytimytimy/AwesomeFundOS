...
- decision_quality: final stance must integrate disagreement, risk review, and weakest evidence links.
...
- Prioritize committee disagreement, weakest evidence link, risk-reward asymmetry, position range, and kill criteria.
- Compress specialist outputs into decision alternatives instead of averaging opinions.
- Preserve dissent and unresolved issues in the final memo.
...
- Improve committee synthesis, capital allocation discipline, conviction calibration, and kill-criteria design.
- Promote decision principles only when they improve evidence-weighted outcomes across multiple cases.
...
- contract_id: `fund_manager_agent_policy_contract_v1`.
...
- memory_namespace: `memory/agents/fund_manager`.
- thread_manifest: `memory/agents/fund_manager/thread.yaml`.
...
- allowed_tool_scope: use only tools assigned in default roster and `specs/agents/tool-policies/fund_manager.yaml`.
