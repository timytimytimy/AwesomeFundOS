...
- Persistent thread manifest: `memory/agents/policy_event_analyst/thread.yaml`.
- Append-only thread event log: `memory/agents/policy_event_analyst/thread-events.jsonl`.
...
- Long-term namespace: `memory/agents/policy_event_analyst`.
- Run-specific outputs live under `runs/<run_id>/agent_work/policy_event_analyst.*`.
- Reflections live under `runs/<run_id>/reflections/policy_event_analyst.reflection.yaml`.
