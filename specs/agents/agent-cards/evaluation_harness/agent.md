...
- Persistent thread manifest: `memory/agents/evaluation_harness/thread.yaml`.
- Append-only thread event log: `memory/agents/evaluation_harness/thread-events.jsonl`.
...
- Long-term namespace: `memory/agents/evaluation_harness`.
- Run-specific outputs live under `runs/<run_id>/agent_work/evaluation_harness.*`.
- Reflections live under `runs/<run_id>/reflections/evaluation_harness.reflection.yaml`.
