...
class FundosCliTests(unittest.TestCase):
    ...
    def test_run_topic_creates_complete_workspace(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("portfolio_outcome_summary", os_manifest)
            portfolio_summary = os_manifest["portfolio_outcome_summary"]
            self.assertEqual(portfolio_summary["watchlist_items"], 1)
            self.assertEqual(portfolio_summary["paper_actions"], 1)
            self.assertEqual(portfolio_summary["reviewed_actions"], 1)
            self.assertEqual(portfolio_summary["outcome_status"], "missing_market_replay")
    ...
    def test_inspect_run_prints_operating_system_manifest_summary(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("agent_performance_score=", inspect.stdout)
            self.assertIn("agent_governance_score=", inspect.stdout)
            self.assertIn("evaluation_overall_score=", inspect.stdout)
    ...
    def test_eval_and_evolve_can_reprocess_run(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            evaluation_after_evolve = load_yaml(run_path / "evaluations/evaluation-report.yaml")
            ...
            self.assertIn("agent_performance", evaluation_after_evolve["accepted_outputs"])
            self.assertEqual(evaluation_after_evolve["dimension_scores"]["agent_performance"], performance["average_final_score"])
            self.assertEqual(evaluation_after_evolve["agent_performance_quality"]["agent_count"], performance["agent_count"])
            ...
            self.assertIn("governance_action_counts", os_manifest["agent_governance_summary"])
            self.assertEqual(os_manifest["evaluation_summary"]["overall_score"], evaluation_after_evolve["overall_score"])
            self.assertEqual(os_manifest["evaluation_summary"]["agent_performance_score"], evaluation_after_evolve["dimension_scores"]["agent_performance"])
            self.assertEqual(os_manifest["evaluation_summary"]["agent_governance_score"], evaluation_after_evolve["dimension_scores"]["agent_governance"])
            self.assertFalse(os_manifest["agent_performance_summary"]["real_trade_allowed"])
            self.assertEqual(os_manifest["agent_governance_summary"]["broker_integration"], "disabled")
            self.assertFalse(os_manifest["evaluation_summary"]["real_trade_allowed"])
            self.assertFalse(os_manifest["real_trade_allowed"])
            self.assertEqual(os_manifest["broker_integration"], "disabled")
